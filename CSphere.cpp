...
inline bool CSphere:: get_ignore_collision() {
...
inline bool CSphere::get_whiteball() { // 흰공(발사되는 공)이 맞는 지 확인하는 정보
...
bool CSphere:: create(IDirect3DDevice9* pDevice, D3DXCOLOR color, float radius ) // Default값 반지름을 가지면서 원할시 반지름 변경 가능.
	{
		if (NULL == pDevice)
			return false;
...
inline bool CSphere:: hasIntersected(CSphere& ball)
	{
...
inline double CSphere:: getVelocity_X() { return this->m_velocity_x; }
inline double CSphere:: getVelocity_Z() { return this->m_velocity_z; }

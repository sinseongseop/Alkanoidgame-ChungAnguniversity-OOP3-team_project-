...
bool Setup()
{
	...
	//player setting
	if (false == white_ball.create(Device, { d3d::WHITE })) return false;
	white_ball.setCenter(white_ball_pos[0], (float)white_ball.getRadius(), white_ball_pos[1]);
	white_ball.setPower(0, 0);
	...
	white_ball.set_whiteball(true); // 충돌에도 안 없어지게 하기 위한 설정
	...
	if (Device)
	{
		...
		for (i = 0; i < 10; i++) {
			...
			if (monster.get_exist()) { // i  실재로 존재하면 i공과 움직이는 공의 충돌 확인
				...
				monsterGenerator.setMonsterExist(i, monster.get_exist());
			}
		}

		g_target_blueball.hitBy(white_ball); // 파란공과 흰공 충돌 확인

		if (white_ball.is_balloutside()) {
			white_ball.destroy();
			if (false == white_ball.create(Device, d3d::WHITE)) return false;
			white_ball.setCenter(white_ball_pos[0], (float)M_RADIUS, white_ball_pos[1]);
			white_ball.setPower(0, 0);
		}
...
LRESULT CALLBACK d3d::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_SPACE:
			...
			D3DXVECTOR3	whitepos = white_ball.getCenter(); // 이부분을 수정하면 발사되는 공이 수정 됩니다.
			...
			white_ball.setPower(distance * cos(theta), distance * sin(theta)); // 앞의 객체를 발사하는 공으로 변경하면 됩니다.
